...
In order to fit the stan model, we need to pass some data: 
...
We can now use the `` function to fit the model. We what two chains, each of length 10'000, and we want to discard the first 50% as warm-up.
```{r}
#| echo: true
#| eval: true
...
Let's consider the logistic regression model.........METTERE LINK 
to the [cardiac dataset](data/cardiac.csv).
