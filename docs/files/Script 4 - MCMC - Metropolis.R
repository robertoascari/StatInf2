...
# Statistical Inference II - Script 4
#_____________________________________
...
# Let's suppose that the posterior is a Gamma(alpha.post, beta.post)
# and let's suppose to know just the kernel.

# Proposal: an Exponential distribution with mean
# equal to the previous value of the chain
...
# The acceptance rate is too small: to obtain a 
# reliable sample from the stationary distribution
# we have to increase the length of the chain and include
# a thin period:
...
# HOMEWORK: 
# Consider the same scenario of the latter example.
# Implement a Metropolis-Hasting algorithm by considering
# an independent MH generating the candidate
# theta_star from an exponential distribution
# with mean X. 
# Consider different values for X to investigate 
# whether it affects the acceptance rate.
