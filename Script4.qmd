...
In this Section, we consider the [cardiac dataset](data/cardiac.csv).
Let's consider $Y_i \sim Bernoulli (\theta_i)$, where the probability of success depends on a quantitative covariate $X$ by means of the logit link function: $$logit(\theta_i) = \log\left(\frac{\theta_i}{1-\theta_i}\right) = \alpha + \beta x_i.$$
