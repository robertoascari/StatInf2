...
Finally, the variance of the approximated normal distribution, namely the **inverse** of $\left. -\frac{\partial^2 \log \pi(\theta | \textbf)}{\partial \theta^2} \right|_{\theta = \tilde{\theta}}$.
...
curve(dnorm(x, post.mode, sqrt(sigma_approx)), add=T, col="red")
```
...
A biostatistician aims to compare two treatments: 
...
$Y_1 \sim Bernoulli(\theta_1)$\\ $Y_2 \sim Bernoulli(\theta_2)$
...
The biostatistician's goal is to study the **log odds ratio** $$\eta = \log \frac{\frac{\theta_1}{1-\theta_1}}{\frac{\theta_2}{1-\theta_2}}.$$ 
It is important to note that while we have obtained the posterior distributions of $\theta_1$ and $\theta_2$, we have not directly computed the posterior distribution of $\eta$.
...
hist(eta, prob=T, xlab=expression(eta), main="Posterior distribution of log OR") lines(density(eta), col="red")
```



Using Monte Carlo sampling, the biostatistician estimates that, with 95\% probability, the log odds ratio falls within the interval
(-3.453943, -2.069884).
